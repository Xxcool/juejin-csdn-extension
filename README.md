...
- 🗂️ **现代双 Tab 仪表盘**：重构为扁平化仪表盘（同步记录 / 偏好设置），顶栏实时展示 CSDN 账号状态胶囊。
...
### 📜 2. 同步本人历史文章
1. 进入掘金创作者中心的 [文章管理列表](https://juejin.cn/creator/content/article/essays?status=all)；
2. 找到目标文章，在操作列点击 **“同步到 CSDN”** 按钮（个人主页文章列表中，该入口位于“更多”菜单内）；
3. 若尚未登录 CSDN，扩展顶栏胶囊会提示登录，登录完成后返回页面即可一键继续同步。
